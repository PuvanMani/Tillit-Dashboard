...
function Category() {
    ...
    const ListCatgory = () => {
        BASE_URL.post("/category/list").then((res) => {
            if (res.data.Status) {
    ...
    const Delete = (id) => {
        BASE_URL.post("/category/delete", { CategoryID: id }).then((res) => {
            let x = res.data.Status ? ListCatgory() : "";
        });
    };
    useEffect(() => {
        ListCatgory();
                ...
                <>
                    <IconButton>
                        <Tooltip title="View" arrow>
                            <Link to={`/category/view/${params.row._id}`}><VisibilityIcon sx={{ color: '#5f49d3' }} /></Link>
                        </Tooltip>
                    </IconButton>
                    <IconButton>
                        <Tooltip title="Edit" arrow>
                            <Link to={`/category/edit/${params.row._id}`}><BorderColorIcon sx={{ color: '#5f49d3' }} /></Link>
                        </Tooltip>
                    </IconButton>
                    <IconButton
                        onClick={() =>
                ...
                >
                    ...
                    >
                        ...
                        >
                            ...
                            <Link to='/category/create'><Button disableFocusRipple disableElevation sx={{ backgroundColor: "#085e15", color: "#FFF", ":hover": { backgroundColor: "#085e15", color: "#FFF" } }}>Add Category</Button></Link>
                        </Box>
                    </Box>
