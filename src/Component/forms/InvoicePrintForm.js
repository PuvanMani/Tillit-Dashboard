...
export default function InvoicePrintForm(props) {
    ...
    const OfficeAddress = (
        <Box>
            <Typography>Zhaa Mart Pvt Ltd,</Typography>
            <Typography>Bharathi Nagar, Tamil Street</Typography>
            <Typography>Kadaiyam Road, Pavoorchatram</Typography>
            <Typography>Tenkasi-627808.</Typography>
        </Box>
    );

    const Read = () => {
        BASE_URL.post('invoice/read', { InvoiceID: params.InvoiceID }).then((res) => {
            if (res.data.result.length > 0) {
                ...
                console.log(JSON.parse(res.data.result[0].Address));
            ...
            <Box ref={(elem) => componentRef = elem} sx={{ background: "#fff", borderRadius: "20px", py: 3, "@media print": { m: 3, pt: 2, } }}>
                <Grid container justifyContent="space-between">
                    ...
                    <Grid item xs={4} sx={{ display: "flex", justifyContent: "end" }}>
                        <Box component='img' src={InvoiceImage} sx={{ height: "95%", width: "40%", mr: "20px", "@media print": { width: "110%" } }} alt='invoice' />
                    </Grid>
                </Grid>
                {/*main  */}
                ...
                <Grid container sx={{ mt: 15, pl: 4 }}>
                    ...
                    <Grid item xs={3.5} sx={{ pl: 6 }}>
                        <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                            <Box component="img" sx={{ width: "60px" }} src={Signature} alt="Signature" />
