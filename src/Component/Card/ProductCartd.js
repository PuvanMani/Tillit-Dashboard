...
export default function ProductCard({ data }) {
    ...
    return (
        <Card sx={{ maxWidth: "100%", boxShadow: "rgba(60, 64, 67, 0.3) 0px 1px 2px 0px, rgba(60, 64, 67, 0.15) 0px 2px 6px 2px;", p: 1 }}>
            <CardMedia
                sx={{ height: "140px", width: "100%", objectFit: "cover", borderRadius: "12px" }}
                image={ImageURL[0].Document}
                title="green iguana"
            />
            <CardContent>
                ...
                <Typography gutterBottom variant="h5" component="h5" sx={{ fontSize: "11px", mb: 0 }}>
                    ...
                    <Typography variant='h6' component='h6' sx={{ fontSize: "13px", color: "#000", my: .4 }}>Price : ₹{YourPrice} <Typography component='span' variant='span' sx={{ color: "red", fontSize: { xs: "9px", sm: "12px" }, textOverflow: "ellipsis" }}>{MarketPrice > YourPrice ? `${(Math.round((((MarketPrice - YourPrice) / MarketPrice) * 100)))}% off` : ""}</Typography></Typography>
